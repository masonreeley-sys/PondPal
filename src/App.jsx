...
export default function App() {
  ...
  useEffect(() => localStorage.setItem("pondpal-dark", JSON.stringify(darkMode)), [darkMode]);
  useEffect(() => { if (user) loadData(); }, [user]);
  useEffect(() => { if (tab === "map") fetchNearbyWaters(); }, [tab, mapRadius]);

  async function loadData() {
    const { data: pondData } = await supabase.from("ponds").select("*").order("created_at", { ascending: true });
    let loadedPonds = pondData || [];

    if (loadedPonds.length === 0) {
      const { data: created } = await supabase
        .from("ponds")
        .insert({ user_id: user.id, name: "Main Pond", location: "Home" })
        .select()
        .single();
    ...
    const { data: catchData } = await supabase.from("catches").select("*").order("created_at", { ascending: false });
  ...
  async function fetchNearbyWaters() {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Map search failed. Try a smaller radius.");
  ...
  async function addNote(e) {
    ...
    const { data, error } = await supabase
      .from("notes")
      .insert({ user_id: user.id, pond_id: selectedPondId, note })
      .select()
      .single();
  ...
  const plan = useMemo(() => {
    ...
    if (goal === "minnows") return [`${fatheads} lb fathead minnows`, "Add PVC, brush piles, rock, or pallets", "Wait before adding predator fish"];
    if (goal === "bass") return [`${bluegill} bluegill/redear mix`, `${fatheads} lb fathead minnows`, `${bass} largemouth bass once forage is ready`];
  ...
  const personalRecords = useMemo(() => {
    ...
    pondCatches.forEach((fish) => {
      ...
      if (!old || score > oldScore) records[species] = fish;
    });
      ...
      <div style={{ ...styles.authPage, background: light.page }}>
        <div style={styles.authCard}>
          ...
          <form onSubmit={handleAuth} style={styles.authForm}>
            ...
            <button style={styles.primaryButton}>{authMode === "login" ? "Log In" : "Create Account"}</button>
          </form>
    ...
    <div style={{ ...styles.app, background: theme.page, color: theme.text }}>
      <aside style={{ ...styles.sidebar, background: theme.sidebar, borderColor: theme.border }}>
        <div>
          ...
          <select style={{ ...styles.pondSelect, background: theme.input, color: theme.text, borderColor: theme.border }} value={selectedPondId} onChange={(e) => setSelectedPondId(e.target.value)}>
      ...
      <main style={styles.main}>
        <header style={styles.topbar}>
          <div>
            ...
            <p style={{ ...styles.subtitle, color: theme.muted }}>{selectedPond?.name || "No pond"} • {selectedPond?.location || "Add a pond"}</p>
          </div>
        </header>
          ...
          <>
            <section style={styles.hero}>
              <div>
                ...
                <p style={styles.heroText}>Track catches, upload fish photos, compare records, and find lakes near Lake Charleston.</p>
              </div>
              <div style={styles.scoreCard}>
                <p style={styles.scoreLabel}>Pond Health Score</p>
                <h3 style={styles.score}>78</h3>
                <div style={styles.progressBack}><div style={styles.progressFill}></div></div>
              </div>
            </section>
        ...
        {tab === "checklist" && (
          <Panel theme={theme}>
            ...
            {["Check water clarity", "Add aeration if oxygen is low", "Add shallow cover for minnows and bluegill", "Stock forage fish first", "Wait before adding bass", "Track fish sizes after stocking"].map((item) => <p key={item} style={styles.check}>✅ {item}</p>)}
          </Panel>
        )}
        ...
        {tab === "map" && (
          <Panel theme={theme}>
            ...
            <p style={{ color: theme.muted, fontWeight: 800, marginTop: 14 }}>
              Found {nearbyWaters.length} named lakes, ponds, or reservoirs. Use smaller radius if 200 miles loads slowly.
            </p>
          </Panel>
        )}
            ...
            <div style={styles.grid}>
              {ponds.map((pond) => (
                <div key={pond.id} style={{ ...styles.card, background: theme.card, borderColor: theme.border }}>
                  ...
                  <p style={{ ...styles.cardTitle, color: theme.muted }}>{pond.location}</p>
            ...
            <div style={styles.noteList}>
              {pondNotes.map((item) => (
                <div key={item.id} style={{ ...styles.noteCard, background: theme.soft, borderColor: theme.border }}>
                  ...
                  <button style={styles.deleteButton} onClick={() => deleteNote(item.id)}>Delete</button>
                </div>
              ))}
            </div>
          </Panel>
            ...
            <div style={{ marginTop: "24px" }}>
              ...
              {aiHistory.map((item) => (
                <div key={item.id} style={{ ...styles.noteCard, background: theme.soft, borderColor: theme.border, marginBottom: "12px" }}>
                  <div><strong>{item.date}</strong><p><b>You:</b> {item.question}</p><p><b>PondPal:</b> {item.answer}</p></div>
                </div>
              ))}
            </div>
          </Panel>
...
function SideButton({ label, icon, active, onClick, theme }) {
  return <button onClick={onClick} style={{ ...styles.sideButton, background: active ? "#0f766e" : "transparent", color: active ? "white" : theme.text }}><span>{icon}</span>{label}</button>;
}

function Card({ title, value, emoji, theme }) {
  return <div style={{ ...styles.card, background: theme.card, borderColor: theme.border }}><div style={styles.cardEmoji}>{emoji}</div><p style={{ ...styles.cardTitle, color: theme.muted }}>{title}</p><h3 style={styles.cardValue}>{value}</h3></div>;
}

function Panel({ children, theme }) {
  return <section style={{ ...styles.panel, background: theme.card, borderColor: theme.border }}>{children}</section>;
}

function RecordTable({ rows, theme, deleteCatch, showAction = false }) {
  if (!rows.length) return <p style={{ color: theme.muted, fontWeight: 800 }}>No catches logged yet.</p>;
  return (
        ...
        <tbody>
          {rows.map((fish) => (
            <tr key={fish.id} style={{ borderTop: `1px solid ${theme.border}` }}>
              ...
              {showAction && <td style={styles.td}><button style={styles.deleteButton} onClick={() => deleteCatch(fish.id)}>Delete</button></td>}
            </tr>
          ))}
        </tbody>
      </table>
...
function LeaderboardTable({ rows, theme }) {
  if (!rows.length) return <p style={{ color: theme.muted, fontWeight: 800 }}>No catches logged for this pond yet.</p>;
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={styles.table}>
        <thead><tr><th style={styles.th}>Rank</th><th style={styles.th}>Species</th><th style={styles.th}>Weight</th><th style={styles.th}>Length</th><th style={styles.th}>Location</th><th style={styles.th}>Date</th></tr></thead>
        <tbody>
          {rows.map((fish, index) => (
            <tr key={fish.id} style={{ borderTop: `1px solid ${theme.border}` }}>
...
const styles = {
  ...
  progressFill: { width: "78%", background: "#bef264", height: "100%" },
